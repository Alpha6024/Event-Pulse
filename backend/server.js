...
// ── Schemas & Models (defined BEFORE use) ──────────────────────────────────
...
app.get("/api/admin/stats", auth(["admin"]), async (req, res) => {
  try {
    ...
    res.json({ students, organizers, totalEvents });
  } catch {
...
app.post("/api/admin/create-user", auth(["admin"]), async (req, res) => {
  try {
    ...
    const newUser = await User.create({ username, password, role, email, name: "", mobile: "" });
...
// ── Events — specific routes BEFORE param routes ───────────────────────────
app.get("/api/events", async (req, res) => {
  res.json(
    await Event.find()
      .populate("organizerId", "username")
      .populate("registeredStudents", "name username email")
  );
});
...
app.delete("/api/events/unregister/:id", auth(["student"]), async (req, res) => {
  try {
    const event = await Event.findById(req.params.id);
    if (!event) return res.sendStatus(404);
